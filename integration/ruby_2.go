...
func testRuby2(t *testing.T, context spec.G, it spec.S) {
	var (
	...
	context("when the buildpack is run with pack build", func() {
		...
		it("installs with the defaults", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "ruby_2"))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(logs).To(ContainLines(
				MatchRegexp(`  rubygems-update version explicitly set to '3\.4\.22'`),
			))
			Expect(logs).To(ContainLines(
